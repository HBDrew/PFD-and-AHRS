...
disp["ds"] = {                      # display settings
    ...
    # Data strip — bare nav label = ACTIVE WAYPOINT, "…D" = FINAL DESTINATION.
    ...
    # PFD top readout ribbon — 5 slots in the band above the AI (tap to config).
...
def _ssync_kinds_from_cs(direction):
    ...
    # aircraft, so every display must agree.  Edit on any screen pushes to all.
...
def _ssync_publish_fpl_lib(force=False):
    """Broadcast this screen's saved-plan + user-waypoint LIBRARY so any
    peer can load a plan stored here.  Rate-limited; pass force=True after
    a save/delete to push immediately."""
...
def _to_disp_brg(true_deg, lat=None, lon=None):
    ...
    MAG subtracts the WMM2025 variation at the reference point (ownship by
    default); TRUE passes through.  Returns degrees in [0, 360)."""
    if true_deg is None:
        return true_deg
    if _hdg_ref() == "true":
...
def _brg_suffix():
    """Course/bearing suffix: "T" in true mode, "" (bare) in mag mode."""
...
def _push_baro_to_pico(qnh_hpa: float):
    ...
    Without this, the PFD shows the entered baro locally but the AHRS-derived
    altitude still reflects the firmware's old QNH — a silent miscalibration.
    The serial path matters because the Pi Zero's primary link is USB: an
    HTTP-only push (the original behaviour) silently fails over USB, so the
    Kollsman knob never moved the displayed altitude.  Mirrors $ALIGN/$MAGDEV.
    Runs in a background thread so the frame loop is never blocked.
    """
    def _worker():
        client = _sse_client
        ...
        # WiFi-only link — push over HTTP /baro instead.
...
def smooth_state():
    ...
    for k in ("lat", "lon", "track", "fix", "sats",
              ...
              "pitch_align", "roll_align",
              "mounting", "orientation", "fw_ver"):
        if k in snap:
            disp[k] = snap[k]
...
def draw_heading_tape(surf, hdg, hdg_bug=None, track=None, gps_ok=False,
    ...
    # Reference annunciation (MAG / TRU) at the box's top-left — amber for TRU
    # to flag the non-default (charts/ATC are magnetic).
    _ref_col = AMBER if ref_label == "TRU" else (150, 170, 190)
    _text(surf, ref_label, 11, _ref_col, x=bx + 4, y=by2 + 3)
...
def handle_event(event, demo_mode):
    ...
    if event.type in (pygame.MOUSEBUTTONUP, pygame.FINGERUP) and _mfd_drag is not None:
        ...
        if not d["is_drag"]:
            ...
            if _ovl_state == "tfc":
                # Traffic page: tap an aircraft for its detail card; airports /
                # METARs / hazards aren't hit-tested here.
    ...
    if event.type in (pygame.MOUSEBUTTONDOWN, pygame.FINGERDOWN):
        ...
        if mode == "ahrs_setup":
            ...
            if action == "back":
                ...
                # Input-side axis alignment — clamp to ±15° (firmware cap),
                # push to the Pico via $ALIGN.
                _, key, delta_str = action.split(":")
...
# ALIGN + the ZERO cage) so the AHRS zero and the display zero aren't mixed.
# Both are ±0.1° trim steppers, drawn by hand rather than via the schema.
_DSP_ROWS_HORIZON = []
...
def draw_display_setup(surf, ds):
    ...
    # HORIZON tab — PITCH / ROLL TRIM (display-side horizon offset), stored in
    # disp["ss"] (the value the AI renderer adds), one ±0.1° stepper per row.
...
def _flight_zero():
    ...
    **input-side axis alignment** (`pitch_align` / `roll_align` — the rotation
    ...
    # Also zero the slip ball at the current (coordinated) lateral accel, so the
    # one cage squares the horizon AND centres the ball to match the panel.
...
def draw_ahrs_setup(surf, ss):
    ...
    # axis alignment — it rotates the raw gyro/accel/mag BEFORE the filter to
    # cancel a mounting tilt.  ZERO auto-captures the current attitude into BOTH
    # align axes (the Sentry/ForeFlight cage) — a true AHRS SENSOR zero, pushed
    # to the Pico via $ALIGN and persisted in its flash.
    # (PITCH/ROLL TRIM — the display-side horizon offset — now live on the
    #  DISPLAY setup HORIZON tab so the AHRS zero and display zero aren't mixed.)
    pico_pa = float(state.get("pitch_align", 0.0))
    ...
    # Row 1: ROLL ALIGN
    pico_ra = float(state.get("roll_align", 0.0))
...
def ahrs_setup_hit(x, y, ss):
    ...
    for ri in range(7):
        ...
        bx = _SS_MX
        if ri in (0, 1):
            # ALIGN steppers on 0/1 (±0.1° per tap); row 0 carries ZERO.
            # (PITCH/ROLL TRIM moved to the DISPLAY setup HORIZON tab.)
            key = "pitch_align" if ri == 0 else "roll_align"
            ry = by + (_SS_RH - _SS_TRIM_H) // 2
            if not (ry <= y <= ry+_SS_TRIM_H):
                continue
            if ri == 0:
...
def _push_ayzero_to_pico():
    """Tell the AHRS to zero the slip ball at the current (coordinated) lateral
    accel — the companion to $ALIGN for the LEVEL/ZERO cage, so one press
    squares the horizon AND centres the ball.  Serial $AYZERO with an HTTP
    /ayzero fallback.  The AHRS persists the offset, so it survives reboots."""
    def _worker():
        client = _sse_client
        if client is not None and hasattr(client, "write"):
            try:
...
def _fpl_reverse_route():
    ...
    has no meaning on the reversed route.  No-op with fewer than two waypoints."""
    wps = disp["fpl"]["waypoints"]
    if len(wps) < 2:
        return
    wps.reverse()
...
def _update_traffic(demo_mode):
    ...
    # sensor) wins.  Split R/I counts let the pilot verify the radio is
    # actually producing targets even in AUTO.
    online = False
    radio = []
    ...
    for t in raw:
        ...
        # Stable closure estimate + arming/latch hysteresis (shared helper) so
        # the visual threat tier doesn't chatter on borderline traffic.
        tid = t.get("icao")
        if tid is not None:
...
def _update_weather():
    ...
    # RADIO is FIS-B only (and pauses the internet pull), INET ignores radio.
    ...
    # — the same carve-out as the local traffic sensor.  Walks every stale zone
    # (aircraft's first), one per tick, then idles until the ~6 h GFS cadence;
    # disk-cached across restarts.
    if _winds_client is not None:
...
def _update_nexrad():
    """Pause/resume the radar poller with the overlay, and decode a new
    image into a pygame surface only when one arrives (seq change)."""
...
def draw_cdi(surf):
    ...
    bar_w = max(140, int(DISPLAY_W * 0.48))   # ~50% wider than the original 0.32
    bar_h = 8
    bar_y = HDG_Y - 56
    bar_x = CX - bar_w // 2
...
def _cdi_hit(x, y):
    ...
    bar_w = max(140, int(DISPLAY_W * 0.48))
    bar_y = HDG_Y - 56
    bar_x = CX - bar_w // 2
...
def _asp_download_thread(sources=None, bucket_label="airspaces"):
    ...
    in the dir.  `sources` defaults to ALL configured sources
    (DOWNLOAD_SOURCES = static + TFR combined).  `bucket_label` shows
    up in the status line so the pilot knows which bucket is running."""
    asp = disp["asp"]
...
def _asp_bucket_mtime(filenames):
    """Return the latest mtime across the given filenames in
    AIRSPACE_DIR, or None if none of them exist on disk."""
    latest = None
...
def _asp_format_date(mtime):
    """Compact date string for the on-screen 'last download' stamp."""
    if mtime is None:
        return "—"
    import datetime as _dt
...
# Each row: class swatch + label + description + ON/OFF segmented control.
...
def draw_airspace_classes(surf):
    ...
    for i, (suffix, lbl, desc) in enumerate(_ASP_CLS_ROWS):
        ...
        pygame.draw.rect(surf, (45, 65, 95), (bx, ry, bw, row_h),
                         width=1, border_radius=6)
        # Class colour swatch
        col, _fill = _mfd_map._AIRSPACE_COLORS.get(
            lbl, _mfd_map._AIRSPACE_DEFAULT)
        ...
        _text(surf, desc, 14, WHITE, bold=True,
              x=sx + sw_w + 12, y=ry + 6)
        # ON/OFF segmented control on the right
...
_MFD_STRIP_AVAILABLE = (
    ...
    ("vs",   "VS",   False, "Vertical speed, fpm"),
    ("oat",  "OAT",  False, "Outside air temperature"),
    ("da",   "DA",   False, "Density altitude, feet"),
    ("pa",   "PA",   False, "Pressure altitude, feet"),
...
# One-time migration of the pre-convention IDs (schema v2): old DIST/ETE were
# waypoint, DISW/ETEW destination, but the ETA pair was reversed.
...
def draw_pfd_top_strip(surf):
    ...
    annunciation paints over it.  Tap it to choose fields (own pfd_top_kinds)."""
...
def _mfd_strip_eta_str(gs_kt, dist_nm, local=False, tz_lat=None, tz_lon=None):
    ...
    point's timezone + DST at the arrival instant (see shared/localtime.py)."""
    if gs_kt < 3.0 or dist_nm <= 0.0:
        return "--:--"
    hours = dist_nm / gs_kt
    if hours >= 99.0:
...
def _mfd_strip_format(kind, ctx):
    ...
    if kind == "btw":
        return (caption,
                f"{int(round(_to_disp_brg(ctx['brg']))) % 360:03d}°", MAGENTA)
    if kind == "dtk":
        return (caption,
                f"{int(round(_to_disp_brg(ctx['dtk']))) % 360:03d}°", MAGENTA)
    # Nav convention: bare = to the ACTIVE WAYPOINT (ctx["dist_nm"]); the
    # "…D" variant = to the FINAL DESTINATION (whole remaining route).
    def _fmt_dist(d_nm):
        return f"{int(round(d_nm)):d}" if d_nm >= 1000.0 else f"{d_nm:.1f}"
...
def _mfd_draw_source_status(surf):
    ...
    if _adsb_client is not None:
        ...
        # Split counts: R = radio/SDR targets, I = internet-feed targets.
    ...
    if _wx_client is not None:
        ...
        # Same R/I split + tap-to-cycle as the ADS-B line.
        live = (w.get("n", 0) > 0
...
def draw_mfd(surf, connected=True, data_stale=False):
    ...
    _mfd_map.render(
        ...
        # _update_traffic.  Clamped to nearby safety traffic on non-TFC
        # overlays so it doesn't clutter the weather / airspace picture.
        ...
        winds_barbs=(_winds_barbs()
                     if (disp["ds"].get("map_show_winds")
                         and range_nm >= WINDS_MIN_RENDER_NM) else None),
...
def _mfd_cycle_winds_alt():
    ...
    i = _WINDS_ALTS.index(cur) if cur in _WINDS_ALTS else 0
    ...
    if levels:
        for lv in levels:
            ...
            def _row(s, x, y, a=lv["alt_ft"], wd=wind, tp=temp):
                ...
                _text(s, wd, 17, (210, 220, 230), x=x + 110, y=y)
                _text(s, tp, 17, (190, 205, 225), x=x + 250, y=y)
            items.append((24, _row))
    else:
        items.append((24, lambda s, x, y: _text(s, "No winds aloft.", 18,
                                                 (200, 215, 230), x=x, y=y)))
    _draw_wx_scroll_panel(surf, title, items, min(560, DISPLAY_W - 20))
...
_MSS_GRID_GAP   = 6
_MSS_GRID_COLS  = 5
# Description sentence + arrival-time toggle below the 5-row grid.
_MSS_DESC_Y     = _MSS_HEADER_H + 8 + _MSS_SLOT_H + 16 + 5 * (56 + _MSS_GRID_GAP) + 2
_MSS_TZ_Y       = _MSS_DESC_Y + 16
...
def _mss_cfg():
    """Which strip the picker edits: MFD bottom strip (default) or PFD top
    ribbon.  Returns (settings_key, default_kinds, slot_count, title)."""
...
def draw_mfd_strip_setup(surf):
    """Chooser overlay reached by tapping the MFD strip or the PFD top ribbon."""
    _key, _def, _cnt, _title = _mss_cfg()
    _screen_header(surf, _title)

    sel = int(disp.get("mss_sel", 0))
    sel = max(0, min(_cnt - 1, sel))
    kinds = _mss_kinds()
    ...
    # Description of the readout in the SELECTED slot.
    sel_kind = kinds[sel]
    local = bool(disp["ds"].get("eta_local", True))
    ...
    _text(surf, "ARRIVAL", 11, (150, 165, 185), bold=True,
...
def mfd_strip_setup_hit(x, y):
    ...
    for i, rect in enumerate(_mss_slot_rects()):
        ...
        if bx <= x <= bx + bw and by <= y <= by + bh:
            return ("slot", i)
    # Kind grid
    for (kind, _cap, _nd2, _d), rect in zip(_MFD_STRIP_AVAILABLE,
                                            _mss_grid_rects()):
        bx, by, bw, bh = rect
        if bx <= x <= bx + bw and by <= y <= by + bh:
            return ("kind", kind)
...
def _fpl_saveload_btns():
    """Return (save_rect, reverse_rect, load_rect) for the route-management row
    beneath the + buttons — three equal thirds spanning the action width."""
...
def draw_fpl(surf):
    ...
    # ── Action row 2: SAVE / REVERSE / LOAD named plans ───────────────
    ...
    if len(wps) >= 2:
        ...
        _text(surf, "REVERSE", 13, (80, 90, 110), bold=True,
    ...
    # ── Action row 3: [approach] [CHANGE] [DEACTIVATE] ────────────────
    # The 3-state approach button (LOAD APPR → ACTIVATE → CANCEL); when an
    # approach is loaded a CHANGE button appears to swap it for a different one.
    ...
    if _phase != "none" and wps:
        ...
        if _legs:
            ...
            for k, (la, lo, ident, _lt, _alt, _at) in enumerate(_legs):
                ...
                # TRK · distance · crossing altitude — right side of the cell,
                # vertically centred (not stacked under the fix name).
                sub = f"{int(round(lb)) % 360:03d}°{_asfx}  ·  {ld:.1f} nm"
                alt_lbl = _appr_alt_label(_alt, _at)
                if alt_lbl:
                    sub += f"  ·  {alt_lbl}"
                _sw = _get_font(17, bold=False).size(sub)[0]
...
def _mfd_find_traffic(tap_x, tap_y, tap_px=30):
    """Nearest drawn ADS-B target within tap_px of the tap, or None.  Builds the
    projector like _mfd_find_metar so the hit-test matches the drawn diamonds."""
...
def _draw_tfc_popup(surf):
    """ADS-B target detail card for the aircraft tapped on the TFC page."""
    t = disp.get("tfc_popup")
    if not t:
        return
    pw = min(560, DISPLAY_W - 20)
    ...
    ph = min(60 + len(rows) * 28 + 30, DISPLAY_H - 16)
    px = (DISPLAY_W - pw) // 2
    py = (DISPLAY_H - ph) // 2
    dim = pygame.Surface((DISPLAY_W, DISPLAY_H), pygame.SRCALPHA)
    dim.fill((0, 0, 0, 150))
...
def _draw_mfd_pick(surf):
    ...
    # Resolve WX live so a post-pan fetch fills the open picker in, and a far
    ...
    if wx is None:
        ...
        icao = wx.get("icao", "WX")
        wx_label = (f"WX  {icao} · {dist:.0f}nm {_compass8(brg)}".rstrip()
...
def render(surf, demo_mode, connected, data_stale=False):
    ...
    # 5. Heading tape — convert hdg/bug/track by the SAME variation so the
    # tape, bug, and track pointer stay mutually consistent (magnetic by
    ...
    # 8b. PFD top readout ribbon — before the badges/banners so alerts override.
...
def main():
    ...
    if disp["cs"].get("adsb_enabled", True):
        ...
        # table read via snapshot(), separate from the GDL90/UDP radio listener
        # so the two sources never co-mingle and RADIO can't be net traffic.
    ...
    if disp["cs"].get("wx_enabled", True):
        ...
        print("[PFD] NEXRAD poller started (radar, follows map view)")
