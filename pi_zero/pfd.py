...
disp["ss"] = {                      # AHRS / sensor settings
    ...
    # subtracted from the Sentry's raw pitch/roll.  Only used when
    # ahrs_source == "sentry".
...
disp["cs"] = {                      # connectivity settings
    ...
    # AHRS source selector (persisted).  "auto" = USB if a Pico is enumerated
    # else WiFi SSE; "usb"/"wifi" force the Pico transport; "sentry" feeds
    # attitude + GPS from a ForeFlight GDL90 source (Sentry) over the ADS-B
    # listener.
...
def _feed_sentry_state():
    ...
    offsets from roll/pitch; a stale sample flips ahrs_ok / gps_ok off."""
    ...
    if event.type in (pygame.MOUSEBUTTONDOWN, pygame.FINGERDOWN):
        ...
        if mode == "ahrs_setup":
            ...
            if action == "back":
                ...
                # zero (the row sub-line then shows the captured P/R).
                a = _adsb_client.ahrs(max_age_s=_SENTRY_AHRS_MAX_AGE_S) \
...
def draw_ahrs_setup(surf, ss):
    ...
    # Row 7: AHRS source — AUTO=Pico (USB/WiFi); USB/WIFI force the Pico;
    # SENTRY feeds a ForeFlight GDL90 receiver (Sentry) over the ADS-B link.
    bx, by, bw, bh = _setting_row(
        surf, 7, "AHRS SOURCE",
        "AUTO/USB/WIFI = Pico    SENTRY = ForeFlight GDL90")
    ...
    # Row 8: Sentry level / zero — a portable GDL90 unit sits at an arbitrary
    # angle, so capture its mounting tilt as the zero (fly level, tap LEVEL).
    # Only meaningful while AHRS SOURCE is SENTRY.
    pz = float(ss.get("sentry_pitch_zero", 0.0))
    rz = float(ss.get("sentry_roll_zero",  0.0))
    if abs(pz) > 0.05 or abs(rz) > 0.05:
        lvl_sub = (f"Sentry zero  P {pz:+.1f}°  R {rz:+.1f}°  "
                   "— LEVEL re-captures, RESET clears")
    else:
        lvl_sub = "Zero the Sentry's mounting tilt — fly level, then tap LEVEL"
    bx, by, bw, bh = _setting_row(surf, 8, "SENTRY LEVEL", lvl_sub)
    lw = 124
    ry = by + (bh - _DSP_BTN_H) // 2
