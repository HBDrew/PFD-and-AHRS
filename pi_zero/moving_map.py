...
def _draw_nexrad_cells(surf, cells, project, rect):
    """Shade FIS-B NEXRAD intensity cells as projected quads (rotation-safe)."""
    x, y, w, h = rect
...
def _draw_metars(surf, metars, rect, lat, lon, cos_lat, cx, cy, px_per_nm,
    ...
    _METAR_LABEL_MAX_NM).  Most stations sit on airports, so this effectively
    colours the field by current conditions.
...
def _metar_label(surf, ix, iy, icao, font):
    ...
    cached (LRU) with a 1 px dark backing baked in for contrast — font.render
    every frame for a screenful of stations is the cost to avoid."""
    if not icao:
        return
    ck = (icao, id(font))
...
# Cached text glyphs for the winds layer — font.render() per barb per frame
# was the dominant inset/MFD cost; distinct temp strings are few, so cache them.
_wx_glyph_cache = {}
...
def render(surf, rect, lat, lon, alt_ft, hdg_deg, track_deg, orient,
    ...
    # Winds page drops the tint whenever WND is selected — NOT only when barbs
    # are loaded — so an empty/loading cache doesn't flicker the tint build on
    # and off.  Barbs read better on a plain background anyway.
    ...
    if runway_marker is not None:
        ...
        if rlabel:
            _place_label(surf, font, rlabel, (210, 220, 235),
                         int(bx2), int(by2), _appr_label_rects,
                         avoid_segs=_appr_avoid)
