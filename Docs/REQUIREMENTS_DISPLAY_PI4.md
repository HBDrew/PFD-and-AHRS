...
> **REQ-DISP-PI4-SPD-007** The airspeed source shall be selectable from the AHRS / Sensors sub-menu. The selectable options shall be `IAS SENSOR` (SDP33-1500Pa with BME280 density correction, sourced from the AHRS `ias_kt` field) and `GPS GS` (GPS groundspeed, sourced from `speed`). The default selection shall be `IAS SENSOR`, with automatic fallback to `GPS GS` when `airdata_ok = False`.
...
- **Air-data integration** (IAS / TAS / wind triangle / stall warn) once the SDP33-1500Pa ships on the new sensor board — see TODO `SDP31-AIRDATA`
