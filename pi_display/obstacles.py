...
# Source: FAA DOF data specification (DAILY_DOF_DAT record layout)
# All offsets are 1-based in the spec; converted to 0-based slices here.
#
# Col  1– 2   OAS Code (state abbreviation, 2 chars)
# Col  3– 9   Obstacle Number (7 chars)
# Col 10      Verification Status (1 char)
# Col 11–14   Country Code (4 chars — "US  " always for DOF)
# Col 15–16   State Code
# Col 17–22   City (6 chars)      ← skip
# Col 23–34   Latitude DMS  (12 chars)
# Col 35–47   Longitude DMS (13 chars)
# Col 48      Obstacle Type Code (1 char) — obsolete, use name below
# Col 49–79   Obstacle Type Name (31 chars)
# Col 80–85   Quantity (6 chars)
# Col 86–91   AGL Height (6 chars, feet)
# Col 92–97   MSL Height (6 chars, feet)
# Col 98      Lighting Code (1 char)
# Col 99      Marking Code (1 char)
# Col 100–112 FAA Study Number (13 chars)   ← skip
# Col 113–118 Action (6 chars)              ← skip
# Col 119–126 Julian Date (8 chars)         ← skip

_COL_LAT_START   = 22   # 0-based
_COL_LAT_END     = 34
_COL_LON_START   = 34
_COL_LON_END     = 47
_COL_TYPE_START  = 48
...
def _parse_record(line: str):
    ...
    """
    # Lines shorter than 98 chars can't contain all required fields
    if len(line) < 98:
        return None
    # Header / comment lines start with spaces or non-digit in col 0
...
def load(data_dir: str):
    ...
    if HAS_NUMPY and os.path.exists(cache_path):
        # Cache is newer than DAT → use it
...
def query_nearby(obstacles, lat: float, lon: float,
    ...
    obstacles : numpy structured array or list of (lat,lon,agl,msl,otype,lit)
    """
    ...
    if HAS_NUMPY and hasattr(obstacles, "dtype"):
        # Bounding-box pre-filter (very fast)
        mask = (
...
def disk_stats(data_dir: str):
    ...
    # Estimate record count from cache size or parse count
    if HAS_NUMPY and os.path.exists(cache_path):
        try:
