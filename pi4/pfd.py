...
disp["ds"] = {                      # display settings
    ...
    # longitude-estimate fallback path (no timezonefinder/zoneinfo); the exact
    # path already knows DST, so this flag is ignored there.  Adds +1 h.
    "dst":              False,
        ...
        for attempt in range(6):
            ...
            if client is not None and hasattr(client, 'write'):
                try:
                    ...
                    client.write(cmd)
                    sent = True
                    print(f"[PFD] align sent via serial (attempt {attempt + 1}) "
                          f"({pitch_align:+.2f},{roll_align:+.2f})")
                except Exception as e:
                    print(f"[PFD] align serial write failed: {e}")
            if not sent:
                # No USB-serial write path (WiFi-only link, e.g. on the Pico W
                # AP) — push the same offsets over HTTP /align so a flight-zero
                # still reaches the AHRS.
                try:
                    base = disp.get("cs", {}).get(
                        "ahrs_url", "http://192.168.4.1").rstrip("/")
                    url = (f"{base}/align?pitch={pitch_align:.2f}"
                           f"&roll={roll_align:.2f}")
                    with urllib.request.urlopen(url, timeout=3) as resp:
                        resp.read()
                    sent = True
                    print(f"[PFD] align sent via HTTP (attempt {attempt + 1}) "
                          f"({pitch_align:+.2f},{roll_align:+.2f})")
                except Exception as e:
                    print(f"[PFD] align HTTP push failed: {e}")
            if not sent:
                print("[PFD] align push: no link to AHRS")
                return
            for _ in range(20):
                _time.sleep(0.1)
                with _state_lock:
...
def _wxloop_available():
    """True when a loop can be built now — internet radar is live (WMS-T can be
    pulled) or FIS-B history exists in the buffer."""
...
# so reopening the loop near the same place reuses ~all frames (only the newest
# 5-min slot is actually new).  RAM only, LRU-capped.
_wxloop_cache = {}                     # (qlat, qlon, qrad, iso) -> (png, bbox)
_WXLOOP_CACHE_MAX = 96                 # ~8 loop-opens' worth
...
def _wxloop_qview():
    ...
    bbox — max cache reuse, and frames overlay consistently during playback."""
...
def _wxloop_start():
    ...
    for i, t in enumerate(ts):                 # instant hits from cache
...
def _wxloop_fetch(qlat, qlon, qrad, ts):
    """Background: fetch each MISSING timeline frame from the WMS-T archive
    (newest first) and cache it.  Cached frames are reused, never re-pulled."""
    for i in range(len(ts) - 1, -1, -1):
        ...
        if len(_wxloop_cache) > _WXLOOP_CACHE_MAX:             # drop oldest
...
def _wxloop_wms_render_arg():
    ...
    fetched PNG on demand (1-entry cache) so RAM stays low."""
    if disp["cs"].get("wx_source", "auto") == "radio":
        return None
...
def _wxloop_draw_controls(surf):
    ...
    for i in range(n):
        ...
        # brighter tick for frames whose WMS image has arrived
    ...
    # "loading" hint while this frame's archived WMS image is still in flight
    if (disp["cs"].get("wx_source", "auto") != "radio"
...
def draw_roll_arc(surf, roll, speed_kt=None):
    ...
    if _srt is not None:
        for _sgn in (1.0, -1.0):
            ...
            _filled_polygon(surf, tri, _SRT_GREEN)
...
def handle_event(event, demo_mode):
    ...
    if event.type in (pygame.MOUSEBUTTONDOWN, pygame.FINGERDOWN):
        ...
        if mode == "pfd" and disp.get("display_mode", "pfd") == "mfd":
            ...
            elif (disp["ds"].get("map_show_nexrad") and not _wxloop.on
                  and _wxloop_available() and _in(_wxloop_loop_btn_rect())):
                _wxloop_start()                  # ▶ LOOP → build + pull WMS-T
        ...
        if mode == "system_setup":
            ...
            # SHUTDOWN/REBOOT confirm (so you can't arm one then trip it later).
...
_SYS_BTN_H     = 54
# Power buttons (SHUTDOWN / REBOOT) are arm-to-confirm: the first tap arms the
# button (shows CONFIRM), a second tap within this window executes.  Prevents a
# stray tap from halting the panel in flight.
_SYS_CONFIRM_S = 4.0


def _system_power(action):
    """Flush settings, then halt or reboot the host.  Runs `sudo -n systemctl
    poweroff|reboot` (the setup script installs a NOPASSWD sudoers drop-in for
    exactly these two).  Non-interactive so it can never hang on a password
    prompt; the -n makes it fail fast if the drop-in is missing."""
    try:
        _settings.flush()
    except Exception as e:
...
def draw_system_setup(surf):
    ...
    half_w = (bw - 10) // 2
    # FIRMWARE first (right after terrain row), then SIMULATOR/RESET, QUIT, then
    # the SHUTDOWN / REBOOT power row (arm-to-confirm).
...
def _local_offset_hours(lat, lon, when_utc=None):
    ...
    Wraps shared/localtime.offset_hours.  On the longitude-estimate fallback
    (no timezonefinder/zoneinfo) offset_hours has NO daylight-saving shift, so
    the pilot-set ``ds["dst"]`` flag adds the missing hour.  On the exact path
    DST is already baked in, so the manual flag is ignored."""
...
def _gps_local_offset_h():
    ...
    if not disp.get("gps_ok"):
        return None
    return _local_offset_hours(disp.get("lat"), disp.get("lon"))
...
def draw_mfd(surf, connected=True, data_stale=False):
    ...
    if ds.get("map_show_nexrad") and (_wxloop.on or _wxloop_available()):
...
def _mss_tz_rects():
    ...
    bw_, bh_, g = 110, 42, 8
    x0 = (DISPLAY_W - (2 * bw_ + g)) // 2 + 40   # nudged right of the label
...
def _mss_dst_rect():
    ...
    the longitude-estimate fallback path — see ds["dst"]."""
...
def draw_mfd_strip_setup(surf):
    ...
    # Manual DST shift — only live on the longitude-estimate fallback and only
    # when showing LOCAL time.  Dim it otherwise.
    ...
    if dst_live:
        ...
        _text(surf, "DST +1h", 15, (80, 95, 120), bold=True,
